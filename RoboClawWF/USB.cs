...
namespace USBLib 
{
	...
	{
		public USBDeviceInfo(string location, string deviceID, string pnpDeviceID, string description)
		{
    ...
    #region "API Region" 
        ...
        const int SPDRP_DEVICEDESC = 0;
        const int SPDRP_HARDWAREID = 0x1;
        const int SPDRP_COMPATIBLEIDS = 0x2;
        const int SPDRP_UNUSED0 = 0x3;
        const int SPDRP_SERVICE = 0x4;
        const int SPDRP_CLASS = 0x7;
        const int SPDRP_CLASSGUID = 0x8;
        const int SPDRP_DRIVER = 0x9;
        const int SPDRP_CONFIGFLAGS = 0xa;
        const int SPDRP_MFG = 0xb;
        const int SPDRP_FRIENDLYNAME = 0xc;
        const int SPDRP_LOCATION_INFORMATION = 0xd;
        const int SPDRP_PHYSICAL_DEVICE_OBJECT_NAME = 0xe;
        const int SPDRP_CAPABILITIES = 0xf;
        const int SPDRP_UI_NUMBER = 0x10;
        const int SPDRP_UPPERFILTERS = 0x11;
        const int SPDRP_LOWERFILTERS = 0x12;
        const int SPDRP_BUSTYPEGUID = 0x13;
        const int SPDRP_LEGACYBUSTYPE = 0x14;
        const int SPDRP_BUSNUMBER = 0x15;
        const int SPDRP_ENUMERATOR_NAME = 0x16;
        const int SPDRP_SECURITY = 0x17;
        const int SPDRP_SECURITY_SDS = 0x18;
        const int SPDRP_DEVTYPE = 0x19;
        const int SPDRP_EXCLUSIVE = 0x1a;
        const int SPDRP_CHARACTERISTICS = 0x1b;
        const int SPDRP_ADDRESS = 0x1c;
        const int SPDRP_UI_NUMBER_DESC_FORMAT = 0x1e;
        const int SPDRP_MAXIMUM_PROPERTY = 0x1f;
        ...
        static DEVPROPKEY DEVPKEY_Device_LocationInfo;
        static DEVPROPKEY DEVPKEY_Device_BusNumber;
        ...
        { 
            ...
            IntPtr h = SetupDiGetClassDevs(ref HostGUID, IntPtr.Zero, IntPtr.Zero, DIGCF_PRESENT | DIGCF_DEVICEINTERFACE); 
        ...
        { 
            ...
            IntPtr h = SetupDiGetClassDevs(IntPtr.Zero, DevEnum, IntPtr.Zero, DIGCF_PRESENT | DIGCF_ALLCLASSES); 
            if (h.ToInt32() != INVALID_HANDLE_VALUE) 
            { 
                IntPtr ptrBuf = Marshal.AllocHGlobal(BUFFER_SIZE); 
                string KeyName; 
		...
		static public List<USBDeviceInfo> GetUSBDevices()
        {
            ...
            DEVPKEY_Device_LocationInfo = new DEVPROPKEY();
            DEVPKEY_Device_LocationInfo.fmtid = new Guid(
                0xa45c254e, 0xdf1c, 0x4efd, 0x80, 0x20, 0x67,
                0xd1, 0x46, 0xa8, 0x50, 0xe0);
            DEVPKEY_Device_LocationInfo.pid = 15;

            DEVPKEY_Device_BusNumber = new DEVPROPKEY();
            DEVPKEY_Device_BusNumber.fmtid = new Guid(
                0xa45c254e, 0xdf1c, 0x4efd, 0x80, 0x20, 0x67,
                0xd1, 0x46, 0xa8, 0x50, 0xe0);
                ...
                for (int i = 0; ; i++)
                {
                    ...
                    if (Marshal.GetLastWin32Error() != ERROR_NO_MORE_ITEMS)
                    {
						...
						if(hardwareids!=""){
							...
							{
								string busdescription = GetStringPropertyForDevice(info, devdata, DEVPKEY_Device_BusReportedDeviceDesc);
                                string buslocation = GetStringPropertyForDevice(info, devdata, DEVPKEY_Device_LocationInfo);
                                ...
                                IntPtr hKey = SetupDiOpenDevRegKey(info, ref devdata, DICS_FLAG_GLOBAL, 0, DIREG_DEV, KEY_READ);
                                if (hKey!=null)
                                {
                                    uint pvSize = 1024;
                                    System.Text.StringBuilder PortName = new System.Text.StringBuilder(1024);
                                    uint pdwType = 0;
                                    int success = RegQueryValueEx(hKey, "PortName", 0, out pdwType, PortName, ref pvSize);
                                    if (success == ERROR_SUCCESS)
                                    {
                                        try
                                        {
                                            System.Text.RegularExpressions.Regex rgx = new System.Text.RegularExpressions.Regex(@"COM\d+");  // "..... (COMxxx)" -> COMxxxx
                                            if (rgx.Matches(PortName.ToString()).Count > 0)
                                            {
                                                comport = rgx.Match(PortName.ToString()).ToString();
                                            }
                                        }
                                        catch (Exception)
                                        {
            ...
            try
            {
                ...
                for (int i = 0; ; i++)
                {
                    ...
                    if (Marshal.GetLastWin32Error() != ERROR_NO_MORE_ITEMS)
                    {
                        ...
                        //Find just FTDI CDC Virtual Comports
                        if (hardwareids != "")
                        {
